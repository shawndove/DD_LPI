...
min_acc <- max_tdv # set minimum accuracy for jaccard distance
samp.size.list <- list()
# loop over groups
for (i in 1:length(gr.stats.list)) {
  ...
  if (is.na(stdev)) { # if standard deviation is NA...
    ...
    samp.size.list[[i]] <- I(((0.8811 + (0.7653*meangr) + (-0.003308*tslength) + (0.606*I(stdev^(1/4))) + (0.1137*meansd) - I(min_acc^(1/6))) / 0.2482)^7)
...
samp.percent.vec <- unlist(samp.percent.vec)
