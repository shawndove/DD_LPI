...
method_fn <- function(grp_data_culled, sample_pop_id_list, msi_real, grp_real, c, m_colnames, n, n_boot, iter_num, samp_size, bootstrap_size, method) {
  ...
  cat(paste0("Creating group indices for the ", method, " method.\n"))
  ...
  if (method=="lr" | method=="my") {
    ...
    # create final group indices from the resample group indices
    grp_final <- lapply(grp_full, function(i) {
  ...
  if (method=="lr" | method=="my") {
    ...
    # create group indices in lambda format for use in creating final confidence intervals
  ...
  samp_list <- lapply(sample_pop_id_list, function(i) {
    ...
    grp_samp <- group_index_fn(spec_samp, c, m_colnames, n=n, n_boot=n_boot)
    
    # create final index
    msi_samp <- group_index_fn(grp_samp, c, m_colnames, n=n, n_boot=n_boot)
    
    if (method=="lr" | method=="my") {
      
      # create final group indices from the resampled group indices
      grp_final_samp <- lapply(grp_samp, function(i) {
        
        # take the column means
        temp <- as.data.frame(t(colMeans(i[,1:c], na.rm=TRUE)))
        
        # add group ID column back in
        temp$GrpID <- i[1,]$GrpID
    ...
    if (method=="lr" | method=="my") {
      ...
      # create lambda version of group indices for msi confidence intervals
    ...
    return(list(grp_samp, grp_ci, msi_samp, msi_ci, grp_final_samp, msi_final_samp))
  ...
  for (i in 1:length(samp_list)) {
    ...
    # add data to lists
    grp_samp_list[[i]] <- samp_list[[i]][[1]]
    grp_ci_list[[i]] <- samp_list[[i]][[2]]
    msi_samp_list[[i]] <- samp_list[[i]][[3]]
    msi_ci_list[[i]] <- samp_list[[i]][[4]]
    grp_final_samp_list[[i]] <- samp_list[[i]][[5]]
    msi_final_samp_list[[i]] <- samp_list[[i]][[6]]
  ...
  #plot sampled group trends
  for (i in 1:bootstrap_size) {
    
    for (j in 1:length(grp_final)) {
      
      plot_grp <- grp_final_samp_list[[i]][j][[1]][,1:c]
      ...
      lines(m_colnames, plot_ci[2,], lty=2, lwd=2)
      lines(m_colnames, plot_ci[1,], lty=2, lwd=2)
      lines(m_colnames, plot_real, lty=1, lwd=3, col="red")
      grid(col="grey85", lty=2)
      mtext(text = "Year", side = 1, line = 4, cex = 1.5)
      mtext(text = "Index", side = 2, line = 3.5, cex = 1.5)
